...
path_for_dmp      = "C:/Users/piedp/OneDrive/Labo/Sketch/Script/BlendLap/blendshape6.json"
...
def client_handler(c_sock): 
    ...
    if   dataIn["mode"] == "dmpOnly" :
        ...
        q    =  LaplacianSurfaceEdit(path_for_lapEdit).calc_lapEdit(dataIn)
        w    =  q.flatten()
...
class LaplacianSurfaceEdit() :
    ...
    def __init__(self, path_for_lapEdit ) :
        
        self.make_data(path_for_lapEdit)
        

    # u が与えられなくても計算できる範囲についてはあらかじめ計算する。
    def make_data(self, path=-1) :

        connection = []
        with open("C:/Users/piedp/OneDrive/Labo/Sketch/Script/BlendLap/blendshape6.json","r") as json_file:
        ...
        if path != -1 : self.path = path
        path = self.path
        #if connection != -1 : self.connection = connection
        ...
        with open(path, 'r') as json_file:
            ...
            for i in range(n) : 
                ...
                if type(D_dash) is int :
                    ...
                    D_dash  = np.concatenate([D_dash,D_dash_i])
                    #print (D_dash.shape)
                #print (i)
    ...
    def calc_lapEdit(self, inputData):
        
        # 入力データの整理
        path = self.path
        u_index = np.array( inputData["u_index"] )
        moves   = np.array( inputData["moves"] )    #WORLD
        num_of_pin = len(u_index)
        lamda   = inputData["lamda"]
        lamda_2 = np.sqrt(lamda)
        num_of_pin = inputData["num_of_pin"]
